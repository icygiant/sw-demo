...
, login_data as (
    select
        f.user_id
        , count(distinct f.event_time) as login_count
    ...
    where
        ...
        and f.event_time < i.install_time + interval '4 weeks'
    group by
        f.user_id
)
...
, payment_data as (
    select
        f.user_id
        , count(distinct f.event_time) as payment_count
    from sw_test.fact_table f
    join first_install i on f.user_id = i.user_id
    where
        f.event_name = 'payment'
        and f.event_time >= i.install_time
        and f.event_time < i.install_time + interval '4 weeks'
    group by
        f.user_id
