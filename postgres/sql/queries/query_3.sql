...
, payment_data as (
    select
        ...
        , extract(day from (f.event_time - i.install_time)) + 1 as day_of_life
...
, payment_sums as (
    ...
    join sw_test.mvw_fact_with_conversions p2
        ...
        and p2.event_time <= pd.payment_time + interval '7 days'
    join
        sw_test.mvw_fact_with_conversions p3
        on pd.user_id = p3.user_id
        and p3.event_time > pd.payment_time
        and p3.event_time <= pd.payment_time + interval '30 days'
    group by
        pd.user_id,
        pd.day_of_life
)
